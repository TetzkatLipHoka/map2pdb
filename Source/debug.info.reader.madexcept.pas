unit debug.info.reader.madexcept;
...
//
// -----------------------------------------------------------------------------
// Clean-room reader for the map blob that madExcept embeds into a PE image as
// resource "MAD"/"EXCEPT" (and, standalone, as a .mad file). The blob layout:
//
//   'www.madshi.net' (14 bytes)
//   Int32  payloadLength
//   Int32  version                 (2 or 3; only 2/3 non-"minDebugInfo" here)
//   [version 3 only: Word,Word,Int32 crc,Int32 flags]
//   payload[payloadLength]         Blowfish-encrypted, then madZip-compressed
//
// The encryption is Blowfish (standard pi-derived P/S constants) in madExcept's
// "old" block layout; the key is derived from the payload length by
// CalcCryptPassword. The decrypted payload is a 12-byte header
// (Int64 uncompressed size, Int32 crc32) followed by a raw DEFLATE stream.
//
// The inflated data is a simple record stream:
//   Segments: Int32 count; { Int32 rva; Int32 len; PackedString unitName }
//   Publics : Int32 count; { PackedString name; first: Int32 addr,
//                            rest: prev + PackedInt delta }
//   Lines   : Int32 count; first: Int32 line, Int32 addr,
//                          rest: line += PackedInt, addr += PackedInt
//   [optional trailing Int32 entrypoint]
//
// madExcept only stores CODE ranges (one per unit), which map directly onto
// map2pdb modules within a single synthesized .text segment.
// -----------------------------------------------------------------------------
...
type
  TDebugInfoMadExceptReader = class(TDebugInfoReader)
  private
    procedure ParseDebugInfo(DebugInfo: TDebugInfo; const Data: TBytes);
  public
    procedure LoadFromStream(Stream: TStream; DebugInfo: TDebugInfo); override;
  end;

resourcestring
  sMadNotMadExcept = 'Not a madExcept map blob (missing "www.madshi.net" descriptor)';
  sMadUnsupportedVersion = 'Unsupported madExcept map version: %d';
  sMadMinDebugInfo = 'madExcept "MinDebugInfo" blobs are not supported (no symbol names)';
  sMadDecompressFailed = 'Failed to decompress madExcept map payload';
...
uses
  System.Math,
  System.ZLib,
  debug.info.reader.map; // DemangleMapSymbol

{$RANGECHECKS OFF}
{$OVERFLOWCHECKS OFF}

const
  MadDescriptor: AnsiString = 'www.madshi.net';

// -----------------------------------------------------------------------------
//      Blowfish (standard pi constants), madExcept "old" block layout
// -----------------------------------------------------------------------------
const
  CPArr: array[0..17] of Cardinal =
  ...
  CSArr: array[0..3, 0..255] of Cardinal =
...
type
  ...
  strict private
    ...
    function F(X: Cardinal): Cardinal; inline;
  public
    procedure InitKey(Key: PByte; KeyLen: Integer);
    // madExcept crypt2dwords: Encrypt walks P[0..17], Decrypt walks P[17..0]
    procedure Crypt2Dwords(var Dw1, Dw2: Cardinal; Encrypt: Boolean);
    // Decrypt a buffer in place, "old" layout, full 8-byte blocks only
    procedure DecryptOld(Buf: PByte; Len: Integer);
  end;

function TBlowfish.F(X: Cardinal): Cardinal;
begin
  Result := ((S[0, (X shr 24) and $FF] + S[1, (X shr 16) and $FF]) xor
              S[2, (X shr 8) and $FF]) + S[3, X and $FF];
end;

procedure TBlowfish.Crypt2Dwords(var Dw1, Dw2: Cardinal; Encrypt: Boolean);
var
  X1, X2: Cardinal;

  function Pv(K: Integer): Cardinal;
  begin
    if Encrypt then
      Result := P[K]
    else
...
begin
  X1 := Dw1 xor Pv(0);
  X2 := Dw2 xor F(X1) xor Pv(1);
  X1 := X1 xor F(X2) xor Pv(2);
  X2 := X2 xor F(X1) xor Pv(3);
  X1 := X1 xor F(X2) xor Pv(4);
  X2 := X2 xor F(X1) xor Pv(5);
  X1 := X1 xor F(X2) xor Pv(6);
  X2 := X2 xor F(X1) xor Pv(7);
  X1 := X1 xor F(X2) xor Pv(8);
  X2 := X2 xor F(X1) xor Pv(9);
  X1 := X1 xor F(X2) xor Pv(10);
  X2 := X2 xor F(X1) xor Pv(11);
  X1 := X1 xor F(X2) xor Pv(12);
  X2 := X2 xor F(X1) xor Pv(13);
  X1 := X1 xor F(X2) xor Pv(14);
  X2 := X2 xor F(X1) xor Pv(15);
  Dw2 := X1 xor F(X2) xor Pv(16);
  Dw1 := X2 xor Pv(17);
end;

procedure TBlowfish.InitKey(Key: PByte; KeyLen: Integer);
var
  c1, c2: Integer;
  c3, c4: Cardinal;

  function KeyByte(Index: Integer): Cardinal;
  begin
    Result := (PByte(Key) + (Index mod KeyLen))^;
  end;

begin
  Move(CPArr, P, SizeOf(P));
  Move(CSArr, S, SizeOf(S));

  c2 := 0;
  for c1 := 0 to 17 do
  begin
    P[c1] := P[c1] xor ((KeyByte(c2) shl 24) or (KeyByte(c2 + 1) shl 16) or
                        (KeyByte(c2 + 2) shl 8) or KeyByte(c2 + 3));
    c2 := (c2 + 4) mod KeyLen;
  end;
...
procedure TBlowfish.DecryptOld(Buf: PByte; Len: Integer);
var
  i: Integer;
  P8: PByte;
  Lo, Hi: PCardinal;
begin
  ...
  begin
    // "old" layout: crypt2dwords(*(p+4), *(p)) - the +4 dword is Dw1/xL
    Lo := PCardinal(P8);
    Hi := PCardinal(P8 + 4);
    Crypt2Dwords(Hi^, Lo^, False);
    Inc(P8, 8);
  end;
end;

// -----------------------------------------------------------------------------
//      madExcept key derivation (CalcCryptPassword) and container decode
// -----------------------------------------------------------------------------

// Blowfish-encrypt an AnsiString in place, "old" layout, using itself as key
// (mirrors madCrypt.OldEncrypt(result, result)).
procedure OldEncryptSelf(var Data: AnsiString);
var
  Key: AnsiString;
  Bf: TBlowfish;
  i: Integer;
...
begin
  UniqueString(Data);
  Key := Data; // snapshot before mutation (key schedule reads original bytes)
  UniqueString(Key);

  Bf.InitKey(PByte(Key), Length(Key));

  P8 := PByte(Data);
  for i := 0 to (Length(Data) shr 3) - 1 do
  begin
    Bf.Crypt2Dwords(PCardinal(P8 + 4)^, PCardinal(P8)^, True);
    Inc(P8, 8);
  end;
end;

function IntToHexPadUpper(Value: Cardinal; MinDigits: Integer): AnsiString;
begin
  Result := AnsiString(IntToHex(Value, MinDigits)); // uppercase, zero padded
  Result := '$' + Result;                            // madExcept IntToHexEx prefix
end;

function IntToDecPad(Value: Cardinal; MinDigits: Integer): AnsiString;
begin
  Result := AnsiString(Format('%.*d', [MinDigits, Value]));
end;

// Mirrors madMapFile.CalcCryptPassword(len)
function CalcCryptPassword(Len: Integer): AnsiString;
var
  s1, s2: AnsiString;
begin
  s1 := 'madshi' + IntToHexPadUpper(Cardinal(Len), 7);
  s2 := 'madshi' + IntToDecPad(Cardinal(Len), 8);
  Result := s1 + s2;
  Result := Result + Result;
  OldEncryptSelf(Result);
end;

// -----------------------------------------------------------------------------
//      Little-endian cursor over the inflated record stream
// -----------------------------------------------------------------------------
type
  TByteCursor = record
    Data: PByte;
    ...
    function ReadPackedInt: Integer;
...
function TByteCursor.ReadCardinal: Cardinal;
begin
  Result := PCardinal(Data + Pos)^;
  Inc(Pos, 4);
end;

function TByteCursor.ReadInteger: Integer;
begin
  Result := PInteger(Data + Pos)^;
  Inc(Pos, 4);
end;

function TByteCursor.ReadByte: Byte;
begin
  Result := (Data + Pos)^;
  Inc(Pos);
end;

function TByteCursor.ReadPackedInt: Integer;
var
  B: Byte;
begin
  B := ReadByte;
...
function TByteCursor.ReadPackedString: string;
var
  Len: Byte;
  Raw: TBytes;
begin
...
begin
  ...
  begin
    ...
    if (Len = 0) then
    begin
      Logger.Warning('[%6d] Empty module skipped: %s at %.8X', [i, UnitName, Rva]);
      continue;
    end;
    ...
    if (Module <> nil) then
      Module.SourceFiles.Add(UnitName + '.pas');
  end;
  ...
  begin
    ...
    begin
      ...
      if (Symbol = nil) then
        ...
        Logger.Debug('  Symbol[%6d]: %.8X %s, Module: %s', [i, Address, Name, Module.Name]);
    end else
      Logger.Warning('[%6d] Failed to locate module for symbol %s at %.8X', [i, Name, Address]);
  end;
  ...
  begin
    ...
    begin
      if (i = 0) then
      begin
        LineNumber := Cursor.ReadInteger;
        LineAddr := Cursor.ReadCardinal;
      end else
      begin
        Inc(LineNumber, Cursor.ReadPackedInt);
      ...
      begin
        ...
        Logger.Warning('[%6d] Failed to locate module for line %d at %.8X', [i, LineNumber, LineAddr]);
    end;
  end;
...
begin
  ...
  SetLength(Blob, Stream.Size - Stream.Position);
  ...
  case Version of
    ...
    raise EDebugInfo.CreateFmt(sMadUnsupportedVersion, [Version]);
  end;
  ...
  // --- Decompress: 12-byte header (Int64 size, Int32 crc) + raw DEFLATE ----
  ...
  Logger.Info('Inflated %.0n bytes of madExcept debug data', [UncompressedSize * 1.0]);

  ParseDebugInfo(DebugInfo, Inflated);
end;
