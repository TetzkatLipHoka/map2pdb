...
const
  sPrefix = 'Line numbers for ';
var
  Reader: TLineReader;
  LegacyMapFile: Boolean;
  n: integer;
  SegmentID: Cardinal;
  ...
  SegmentName: string;
  ClassName: string;
  ConflictingSegment: TDebugInfoSegment;
  ...
  EndOfName: integer;
  IsPath: Boolean;
  FilenameIsAlias: Boolean;
  ModuleName: string;
  Module: TDebugInfoModule;
  Overlap: TDebugInfoOffset;
  Name: string;
  Symbol: TDebugInfoSymbol;
  Filename: string;
  SourceFile: TDebugInfoSourceFile;
  LineNumber: Cardinal;
  ModuleByOffset: TDebugInfoModule;
    ...
    try
      ...
      *)
      ...
      begin
        ...
        OffsetSize := ParseSegmentAndOffset(Reader.LineBuffer, n, SegmentID, Offset);
        ...
        begin
          ...
          ConflictingSegment := DebugInfo.Segments.FindByOffset(Offset);
        ...
        SegmentClass := TDebugInfoSegment.GuessClassType(ClassName);
        Segment := DebugInfo.Segments.Add(SegmentID, SegmentName, SegmentClass);
        ...
        OverlappingSegment := Segment.FindOverlap;
      ...
      *)
      ...
      while (not Reader.CurrentLine.IsEmpty) do
      begin
        ModulePos := FindRequiredMarker(' C=', Reader.LineBuffer, 1, 'Missing segment class name marker', True);
        n2 := FindRequiredMarker(' ', Reader.LineBuffer, ModulePos, 'Missing segment type separator'); // Here ' ' is considered the marker of the end of the class name
        ClassName := Copy(Reader.LineBuffer, ModulePos, n2-ModulePos);
        Address := Copy(Reader.LineBuffer, 1, ModulePos-4);
        ...
        IsPath := (n2 >= StartOfName) and CharInSet(Reader.LineBuffer[n2], ['\', '/']);
        FilenameIsAlias := (n2 >= StartOfName) and (Reader.LineBuffer[n2] = '|');

        StartOfName := ModulePos;

        ModuleName := Copy(Reader.LineBuffer, StartOfName, EndOfName-StartOfName+1);
        ...
        Size := HexToInt32(Address, ModulePos);
        if (Size = 0) then
        ...
        begin
          ...
          begin
            ...
            Overlap := Module.Offset + Module.Size - Offset;
          ...
          begin
            ...
            Overlap := Offset + Size - Module.Offset;
      ...
      *)
      ...
      while (not Reader.CurrentLine.IsEmpty) do
      begin
        n := FindRequiredMarker(' ', Reader.LineBuffer, 1, 'Missing address/symbol separator');
        Address := Copy(Reader.LineBuffer, 1, n-1);

        Name := Copy(Reader.LineBuffer, n+1, MaxInt).TrimLeft;
        ...
        begin
          ...
          begin
            ...
            Symbol := Module.Symbols.Add(Name, Offset);
            if (Symbol = nil) then
      ...
      *)
      ...
      begin
        ...
        n := FindRequiredMarker('(', Reader.LineBuffer, 1, 'Source file start marker "(" not found');
        ModuleName := Copy(Reader.LineBuffer, Length(sPrefix)+1, n-1-Length(sPrefix));

        n2 := FindRequiredMarker(')', Reader.LineBuffer, n+1, 'Source file end marker ")" not found');
        Filename := Copy(Reader.LineBuffer, n+1, n2-n-1);
        ...
        SegmentName := GetSectionName(Reader.LineBuffer, n);
        Segment := DebugInfo.Segments.FindByName(SegmentName);
        if (Segment = nil) then
          LineLogger.Error('Unknown segment name: %s', [SegmentName]);

        Module := DebugInfo.Modules.FindByName(ModuleName, Segment);
        if (Module <> nil) then
        begin
          SourceFile := Module.SourceFiles.Add(Filename);
          ...
          begin
            ...
            begin
              // Get line number
              LineNumber := DecToInt32(Reader.LineBuffer, n);
              ...
              begin
                ...
                begin
                  // Validate module
                  ModuleByOffset := DebugInfo.Modules.FindByOffset(Module.Segment, Offset);
